...
from cartoview2.core.authorizations import Authorization
from cartoview2.core.serializers import HTMLSerializer, MultipartFormSerializer
...
class BaseModelResource(ModelResource):
    ...
    def new_item(self, request, **kwargs):
        ...
        form = self.get_form()
        return render(request,'rest_api/edit.html',{'form': form, 'operation': 'add', 'resource_uri' : self.get_resource_uri()})

    def edit(self, request, pk, **kwargs):
        self.method_check(request, allowed=['get'])
        obj = self._meta.object_class.objects.get(id=pk)
        form = self.get_form(obj)
        return render(request,'rest_api/edit.html',{'form':form , 'operation': 'edit', 'resource_uri' : self.get_resource_uri()})
